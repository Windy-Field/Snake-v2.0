...
int is_barrier_conflict();  // 障碍刷新辅助函数（保证障碍不会刷到蛇身上且障碍刷新时不会重叠）
...
int draw_map(){
    ...
    int time = (int)current_time - start_time;  // 计算游戏时间
    // 等价于打印一个空心矩形，边框为 #
    ...
    if (self_crash == 1 && through_wall == 1){
        printf("Time: %02d:%02d\nDifficulty: %d; Points: %d\nCrashing self dies.\nCannot go through the walls.\n", minutes, seconds, difficulty, score);
    }
...
int main(){
    ...
    else if (difficulty == 3) difficulty_speed = 1000 / 30;
    else if (difficulty == 4) difficulty_speed = 1000 / 60;
    else {
        difficulty = 2;
